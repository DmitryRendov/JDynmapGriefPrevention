...
public class JDynmapGriefPrevention
  extends JavaPlugin
{
  ...
  private boolean isVisible(String owner, String worldname)
  {
    if ((visible != null) && (visible.size() > 0) && 
      (!visible.contains(owner)) && (!visible.contains("world:" + worldname)) && 
      (!visible.contains(worldname + "/" + owner))) {
      return false;
    }
    
    if ((hidden != null) && (hidden.size() > 0) && (
      (hidden.contains(owner)) || (hidden.contains("world:" + worldname)) || (hidden.contains(worldname + "/" + owner)))) {
      return false;
    }
  ...
  private void handleClaimUuid(int index, Claim claim)
  {
    ...
    if (this.getContainers) {
      ...
      for (int i = 0; i < containers.size(); i++) {
        ...
        if ((this.publicenabled) && 
          (((String)containers.get(i)).equals("public"))) {
          publictrust = true;
        }
  ...
  private void handleClaimName(int index, Claim claim)
  {
    ...
    if (!oowner.equalsIgnoreCase("somebody"))
    {
      ...
      ownerdays = getDayDifference(lastLogin);
      if ((op == null) && (!claim.isAdminClaim()) && (!oowner.equalsIgnoreCase("an administrator")) && (oowner.charAt(0) != '['))
      {
        ownerdays = 666;
      }
      if (ownerdays > absenceDayLimit) {
  ...
  private void updateClaims()
  {
    ...
    int j = (arrayOfOfflinePlayer1 = offlinePlayers).length; for (int i = 0; i < j; i++) { OfflinePlayer op = arrayOfOfflinePlayer1[i];
    ...
    catch (NoSuchFieldException localNoSuchFieldException1) {}catch (IllegalArgumentException localIllegalArgumentException1) {}catch (IllegalAccessException localIllegalAccessException1) {}
    ...
    new BukkitRunnable()
    {
      public void run() {
        ...
        if (JDynmapGriefPrevention.dynmap != null)
        {
          ...
          if ((JDynmapGriefPrevention.this.useDynmap) && 
            (JDynmapGriefPrevention.this.showcountonlayer) && (JDynmapGriefPrevention.this.usetwolayers)) {
            JDynmapGriefPrevention.set.setMarkerSetLabel(JDynmapGriefPrevention.layerUsedName + " (" + JDynmapGriefPrevention.this.countused + ")");
            JDynmapGriefPrevention.setunused.setMarkerSetLabel(JDynmapGriefPrevention.layerUnusedName + " (" + JDynmapGriefPrevention.this.countunused + ")");
          }
        }
  ...
  private void activate()
  {
    ...
    if (!reload)
    {
      text = MessageFormat.format(jdgpMessages.getString("jdgp.activated"), new Object[] { getDescription().getVersion() });
      console(text);
    }
    reload = false;
  }
  ...
  private void copy(String fileFromJar, String outfile)
  {
    // Byte code:
    //   0: ldc 1
    //   2: invokevirtual 1367	java/lang/Class:getClassLoader	()Ljava/lang/ClassLoader;
    //   5: aload_1
    //   6: invokevirtual 1371	java/lang/ClassLoader:getResourceAsStream	(Ljava/lang/String;)Ljava/io/InputStream;
    //   9: astore_3
    //   10: aconst_null
    //   11: astore 4
  ...
  private void checkForUpdate()
  {
    ...
    case UPDATE_AVAILABLE: 
      ...
      text = MessageFormat.format(jdgpMessages.getString("updater.new_version"), new Object[] { checkedPluginVersion });
      console(text);
  ...
  private void readConfigOptions()
  {
    ...
    infowindow = cfg.getString("infowindow", "div class=\"infowindow\">Claim Owner: <span style=\"font-weight:bold;\">%owner%</span><br/>Permission Trust: <span style=\"font-weight:bold;\">%managers%</span><br/>Trust: <span style=\"font-weight:bold;\">%builders%</span><br/>Container Trust: <span style=\"font-weight:bold;\">%containers%</span><br/>Access Trust: <span style=\"font-weight:bold;\">%accessors%</span></div>");
    ...
    newswindow = cfg.getString("newswindow", "div class=\"infowindow\">Claim Owner: <span style=\"font-weight:bold;\">%owner%</span><br/>Permission Trust: <span style=\"font-weight:bold;\">%managers%</span><br/>Trust: <span style=\"font-weight:bold;\">%builders%</span><br/>Container Trust: <span style=\"font-weight:bold;\">%containers%</span><br/>Access Trust: <span style=\"font-weight:bold;\">%accessors%</span></div>");
  ...
  private void setLocale()
  {
    ...
    try {
      ...
      text = MessageFormat.format(jdgpMessages.getString("jdgp.locale.notfound"), new Object[] { this.cfglocale });
      console(text);
      this.cfglocale = "en";
  ...
  private void createPlayerLayer(String name, int claimnumber)
  {
    ...
    if (this.showcountonlayer) {
      playerset = markerapi.createMarkerSet("griefprevention.markersetplayer", name + " (" + claimnumber + ")", null, false);
    } else {
  ...
  private void tpPlayerToClaim(final Player p, final Claim claim)
  {
    ...
    text = MessageFormat.format(jdgpMessages.getString("command.tp.claim.id"), new Object[] { String.valueOf(claim.getID()), claim.getOwnerName() });
    p.sendMessage(text);
    ...
    new BukkitRunnable()
    {
      public void run() {
        ...
        Visualization visualization = Visualization.FromClaim(claim, newloc.getBlockY(), VisualizationType.Claim, newloc);
  ...
  private void tpPlayerToCoordinates(final Player p, int x, int z)
  {
    try
    {
      ...
      text = MessageFormat.format(jdgpMessages.getString("command.tp.coords"), new Object[] { String.valueOf(x), String.valueOf(y), String.valueOf(z) });
      p.sendMessage(text);
  ...
  private void showPlayerClaims(CommandSender sender, String playername)
  {
    ...
    new BukkitRunnable()
    {
      public void run()
      {
        ...
        ArrayList<ClaimInfo> invClaimsList = JDynmapGriefPrevention.this.lookupInvolvedClaims(JDynmapGriefPrevention.mapPlayerName);
        int size = invClaimsList.size();
        if (size > 0)
        {
          JDynmapGriefPrevention.text = MessageFormat.format(JDynmapGriefPrevention.jdgpMessages.getString("command.claims.ok1"), new Object[] { String.valueOf(size) });
  ...
  public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args)
  {
    ...
    {
      if (args.length == 0) {
        text = MessageFormat.format(jdgpMessages.getString("command.help.title"), new Object[] { pluginVersion }) + " (by " + pluginAuthor + ")";
        ...
        if ((dynmap != null) && (this.useDynmap) && (sender.hasPermission("jdynmapgriefprevention.player.claims")))
        {
          sender.sendMessage(jdgpMessages.getString("command.help.claims1"));
        }
        if ((dynmap != null) && (this.useDynmap) && (sender.hasPermission("jdynmapgriefprevention.admin.claims")))
        {
      ...
      if (args.length == 1) {
        ...
        if (args[0].equalsIgnoreCase("reload")) {
          ...
          text = MessageFormat.format(jdgpMessages.getString("command.reload.success2"), new Object[] { Long.valueOf(this.updperiod / 20L) });
          sender.sendMessage(text);
          return true;
        }
        ...
        if (args[0].equalsIgnoreCase("stats")) {
          ...
          text = MessageFormat.format(jdgpMessages.getString("command.stats.claims_owner"), new Object[] { String.valueOf(this.numOwners) });
          sender.sendMessage(text);
          text = MessageFormat.format(jdgpMessages.getString("command.stats.trusted"), new Object[] { String.valueOf(this.numBuilders), 
            String.valueOf(this.numContainers), String.valueOf(this.numAccessors), String.valueOf(this.numManagers) });
          sender.sendMessage(text);
          text = MessageFormat.format(jdgpMessages.getString("command.stats.parent_subdiv"), new Object[] { String.valueOf(this.sz), String.valueOf(this.idx - this.sz) });
          sender.sendMessage(text);
          text = MessageFormat.format(jdgpMessages.getString("command.stats.used"), new Object[] { String.valueOf(this.countused), 
            String.valueOf(this.countadmin), String.valueOf(this.countnormal), String.valueOf(this.countbuilder) });
          sender.sendMessage(text);
          text = MessageFormat.format(jdgpMessages.getString("command.stats.unused"), new Object[] { String.valueOf(this.countunused) });
          sender.sendMessage(text);
          sender.sendMessage(jdgpMessages.getString("command.stats.separator"));
          return true;
        }
      ...
      {
        if (args[0].equalsIgnoreCase("claims")) {
          if (((sender instanceof Player)) && (!sender.hasPermission("jdynmapgriefprevention.admin.claims"))) {
            sender.sendMessage(jdgpMessages.getString("command.noperm"));
            return true;
          }
        ...
        if (args[0].equalsIgnoreCase("tp")) {
          ...
          {
            text = MessageFormat.format(jdgpMessages.getString("command.tp.noclaim.id"), new Object[] { String.valueOf(claimID) });
            sender.sendMessage(text);
            return true;
          }
      ...
      if (args.length == 3)
      {
        if (args[0].equalsIgnoreCase("tp")) {
          ...
          tpPlayerToCoordinates((Player)sender, Integer.valueOf(X).intValue(), Integer.valueOf(Z).intValue());
